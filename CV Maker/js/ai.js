// AI Assistant for CV Maker - Powered by Gemini API
...
function initAIFeatures() {
    ...
    // Event delegation for dynamic AI buttons in experience list
...
// Handle Generate Experience Description
async function handleGenerateExperience(btn) {
    ...
    try {
        ...
        // Fallback: Generate template bullet points
        const fallbackDesc = generateFallbackExperience(jobTitle, company);
        descField.value = fallbackDesc;
        descField.dispatchEvent(new Event('input', { bubbles: true }));

        const message = error.message.includes('403') || error.message.includes('400')
            ? 'API key error. Using template instead.'
...
async function handleGenerateSummary() {
    ...
    try {
        ...
        summaryField.dispatchEvent(new Event('input', { bubbles: true }));
        showAINotification('Used template summary. Check console for API details.', 'warning');
    } finally {
        setButtonLoading(btn, false);
    }
}
