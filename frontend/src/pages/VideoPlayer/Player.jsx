...
const Player = () => {
  ...
  }, [id, requestedVideoId, startGoogleSignIn, location.state]);
  ...
  const fetchTranscriptForActive = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        if (!res.ok) {
          const msg = data?.error || data?.message || "Failed to fetch transcript";
          setTranscript("");
          setErr(msg);
          return;
        }
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-64px)] bg-[color:var(--tf-bg)] overflow-hidden">
      {/* Left: video area */}
      <div className="w-full lg:flex-1 flex flex-col shrink-0 lg:shrink bg-black lg:bg-transparent justify-center lg:justify-start p-0 lg:p-6 overflow-visible">
        ...
        {entry && (
          <div className="p-4 lg:p-0 lg:mt-4 bg-[color:var(--tf-surface)] lg:bg-transparent border-b lg:border-none border-[color:var(--tf-border)]">
            <h2 className="text-lg lg:text-2xl font-bold text-[color:var(--tf-text)] leading-tight line-clamp-2">
              {entry.title}
            </h2>
          </div>
        )}
      ...
      {/* Right: tools */}
      <div className="flex-1 w-full lg:flex-none lg:w-[400px] xl:w-[450px] bg-[color:var(--tf-surface)] shadow-[var(--tf-shadow-md)] border-l border-[color:var(--tf-border)] flex flex-col z-20 overflow-hidden">
        {/* Header / Controls */}
        <div className="p-3 lg:p-6 border-b border-[color:var(--tf-border)] bg-[color:var(--tf-surface-muted)] backdrop-blur-md sticky top-0 z-30">
          {err && (
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 custom-scrollbar bg-[color:var(--tf-bg)]/55">
          {embedUrl && !loading && (
        ...
        {/* Footer / Back Button */}
        <div className="p-3 lg:p-4 border-t border-[color:var(--tf-border)] bg-[color:var(--tf-surface)]">
          <button
            onClick={() => navigate(-1)}
            className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-[color:var(--tf-bg)] text-[color:var(--tf-text-muted)] font-medium rounded-xl shadow-[var(--tf-shadow-sm)] border border-[color:var(--tf-border)] hover:bg-[color:var(--tf-surface-muted)] hover:text-[color:var(--tf-primary)] transition-all duration-200"
          >
            <span>⬅</span>{" "}
