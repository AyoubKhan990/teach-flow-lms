...
export default function LoadingSpinner() {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--tf-bg)] flex flex-col items-center">
      <div className="max-w-7xl mx-auto px-4 py-12 w-full">
        <div className="animate-pulse grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
          {skeletons.map((_, i) => (
            <div
              key={i}
              className="bg-[color:var(--tf-surface)] rounded-xl overflow-hidden shadow-[var(--tf-shadow-sm)] border border-[color:var(--tf-border)]"
            >
        ...
        <div className="text-center mt-8">
          <p className="text-[color:var(--tf-text)] font-semibold">
            Loading your video feed...
          </p>
          <p className="text-sm text-[color:var(--tf-text-muted)] mt-1">
