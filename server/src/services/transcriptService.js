...
class TranscriptSourceError extends Error {
...
async function fetchInvidiousTranscript(videoId) {
  ...
  for (const domain of uniqueInstances) {
    ...
    try {
      ...
      const trackRes = await fetchWithTimeout(trackUrl, { timeoutMs: 8000 });
...
export async function fetchTranscriptText(videoId, lang = "en") {
  ...
  try {
    ...
    errors.push({ source: "youtube-transcript", message: "No transcript items returned." });
  } catch (e) {
    errors.push({ source: "youtube-transcript", message: e.message });
  }
  ...
  try {
    ...
    errors.push({ source: "python", message: "Python returned empty output." });
  } catch (err) {
    errors.push({ source: "python", message: err.message });
  }
  ...
  try {
    ...
    errors.push({ source: "invidious", message: e.message });
  }

  console.error("Transcript fetch failed", { videoId, lang, errors });
  const err = new TranscriptUnavailableError("Transcript not available (all sources failed).", {
    errors,
  });
  throw err;
}
