...
router.get("/:videoId/transcript", async (req, res) => {
  ...
  try {
    ...
    if (e?.name === "TranscriptUnavailableError") {
      const sources = Array.isArray(e.errors) ? e.errors : [];
      const pythonMisconfigured = sources.some(
        (x) =>
          x?.source === "python" &&
          typeof x?.message === "string" &&
          (x.message.includes("ModuleNotFoundError") || x.message.includes("youtube_transcript_api"))
      );
      if (pythonMisconfigured) {
        return res.status(500).json({
          error: "Transcript service misconfigured (missing Python dependency).",
          remediation: "Install youtube-transcript-api in the server Python environment.",
          sources,
        });
      }
      return res.status(404).json({
