...
const variantMeta = {
  info: { icon: Info, className: 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]' },
...
export const ToastProvider = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={{ toast, remove }}>
      {children}
      <div className="fixed right-4 top-4 z-[9999] flex w-[min(420px,calc(100vw-32px))] flex-col gap-2" aria-live="polite" aria-relevant="additions">
        {toasts.map((t) => {
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  {t.title && <div className="text-sm font-semibold text-[color:var(--color-text)]">{t.title}</div>}
                  {t.message && <div className="text-sm text-[color:var(--color-text-muted)]">{t.message}</div>}
                </div>
                <button
                  type="button"
