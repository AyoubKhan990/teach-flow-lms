...
export const useJobProgress = ({ jobId, toast }) => {
  ...
  React.useEffect(() => {
    ...
    fetchJob(jobId)
      ...
      .catch((e) => {
        if (!alive) return;
        setConnecting(false);
        toast({ title: 'Job error', message: e?.message || 'Job not found', variant: 'danger' });
      });
    return () => {
      alive = false;
    };
  ...
  React.useEffect(() => {
    ...
    setConnecting(true);
    es = new EventSource(`http://localhost:5000/api/jobs/${jobId}/events`);
    ...
    es.addEventListener('progress', (ev) => {
      try {
        ...
        if (evt?.stage === 'completed' || evt?.stage === 'failed' || evt?.stage === 'cancelled') {
          fetchJob(jobId)
            .then((data) => {
              if (disposed) return;
              setJob(data.job);
    ...
    const run = async () => {
      while (alive) {
        try {
          ...
          setPollAttempt(0);
          if (data.job?.status === 'completed' || data.job?.status === 'failed' || data.job?.status === 'cancelled') return;
          await sleep(2000);
        } catch {
          if (!alive) return;
  ...
  const retryImages = React.useCallback(async () => {
    await postJobAction(jobId, 'retry-images');
    setMode('sse');
  }, [jobId]);

  const resolveNoImages = React.useCallback(async () => {
    await postJobAction(jobId, 'resolve-no-images');
    await reloadSnapshot();
  }, [jobId, reloadSnapshot]);

  const uploadImages = React.useCallback(
    async (images) => {
      await postJobAction(jobId, 'upload-images', { images });
      await reloadSnapshot();
    },
    [jobId, reloadSnapshot]
  );
