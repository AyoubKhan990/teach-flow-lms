...
export const FormSection = ({ id, title, Icon, subtitle, children, defaultOpen = true }) => {
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            {subtitle && <div className="mt-1 text-sm text-[color:var(--color-text-muted)]">{subtitle}</div>}
          </div>
        </div>
        <div className="mt-1 text-xs font-semibold text-[color:var(--color-text-muted)]">{open ? 'Hide' : 'Show'}</div>
      </button>
      {open && <div id={id} className="px-6 pb-6">{children}</div>}
    </Card>
  );
};
