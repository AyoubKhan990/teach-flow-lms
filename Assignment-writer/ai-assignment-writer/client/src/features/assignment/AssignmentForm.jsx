...
export const AssignmentForm = () => {
  ...
  const {
    ...
    toast,
    onJobCreated: (id) => navigate(`/jobs/${id}`)
  });
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="neutral" className="bg-white/70 backdrop-blur">No signup</Badge>
          <Badge variant="info" className="bg-white/70 backdrop-blur">Export to Word/PDF</Badge>
        </div>
      </header>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-12 lg:items-start">
        <main id="main" className="lg:col-span-8">
          <div className="glass-panel rounded-[var(--radius-hero)] p-6 sm:p-8">
            <h1 className="text-step-2 font-extrabold text-[color:var(--color-text)]">Assignment Builder</h1>
            <p className="mt-2 text-sm text-[color:var(--color-text-muted)]">Provide a topic and choose formatting options. You can include uploaded images or let AI generate visuals.</p>
          </div>
            ...
            {topErrors.length > 0 && (
              <Alert variant="danger" title="Please fix the highlighted fields" role="alert">
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  {topErrors.map((e) => (
                    <li key={e.field}>
                      <button type="button" className="text-left underline underline-offset-2" onClick={() => focusField(e.field)}>
                        {e.message}
                      </button>
                    </li>
                  ))}
